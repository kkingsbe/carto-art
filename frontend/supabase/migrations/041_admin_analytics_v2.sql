-- Update get_activity_stats to include new metrics
...
BEGIN
    ...
    -- Special case: paywall_users (unique users presented with paywall)
    ELSIF p_metric_type = 'paywall_users' THEN
        RETURN QUERY
        SELECT
            date_bin(v_interval, tb.bucket, '1970-01-01') AS bucket,
            count(DISTINCT pe.user_id)::BIGINT -- Count distinct users who saw paywall
        FROM generate_series(
            date_bin(v_interval, p_start_time, '1970-01-01'),
            date_bin(v_interval, p_end_time, '1970-01-01'),
            v_interval
        ...
        LEFT JOIN page_events pe ON 
            pe.event_type = 'paywall_shown' AND
            pe.created_at >= tb.bucket AND 
            pe.created_at < tb.bucket + v_interval
        GROUP BY 1
        ORDER BY 1;

    -- Special case: login_wall_users (unique anonymous sessions presented with login wall)
    ELSIF p_metric_type = 'login_wall_users' THEN
        RETURN QUERY
        SELECT
            date_bin(v_interval, tb.bucket, '1970-01-01') AS bucket,
            count(DISTINCT pe.session_id)::BIGINT -- Count distinct sessions for anon users
        FROM generate_series(
            date_bin(v_interval, p_start_time, '1970-01-01'),
            date_bin(v_interval, p_end_time, '1970-01-01'),
            v_interval
        ...
        LEFT JOIN page_events pe ON 
            pe.event_type = 'login_wall_shown' AND
            pe.created_at >= tb.bucket AND 
            pe.created_at < tb.bucket + v_interval
        GROUP BY 1
        ORDER BY 1;
...
-- Update get_latency_stats to include popup_close_latency
...
BEGIN
    ...
    ELSIF p_metric_type = 'popup_close_latency' THEN
        RETURN QUERY
        SELECT
            date_bin(v_interval, tb.bucket, '1970-01-01') AS bucket,
            -- Average the duration_seconds from metadata
            COALESCE(AVG((metadata->>'duration_seconds')::numeric), 0)::NUMERIC
        FROM generate_series(
            date_bin(v_interval, p_start_time, '1970-01-01'),
            date_bin(v_interval, p_end_time, '1970-01-01'),
            v_interval
        ...
        LEFT JOIN page_events pe ON 
            pe.event_type = 'changelog_close' AND
            pe.created_at >= tb.bucket AND 
            pe.created_at < tb.bucket + v_interval
        GROUP BY 1
        ORDER BY 1;
