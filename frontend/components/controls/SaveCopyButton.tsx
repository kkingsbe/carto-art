...
export function SaveCopyButton({
  ...
  const getDefaultCopyName = (): string => {
    if (currentMapName) {
      return `${currentMapName} - Copy`;
    }
    return 'Untitled Map - Copy';
  };
  ...
  const handleSaveCopyClick = () => {
    ...
    // Always show dialog with pre-filled name
    setProjectName(getDefaultCopyName());
    setError(null);
    setShowDialog(true);
  };
  ...
  const handleSave = async (name: string) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to save copy. Please try again.');
      setIsSaving(false);
    }
  };
  ...
  if (!isAuthenticated) {
    return (
      <Tooltip content="Sign in to save copies">
        <button
          type="button"
          disabled={disabled}
          className={cn(
    ...
    <>
      ...
      >
        ...
        <span className={cn(
          ...
          {isSaving ? 'Saving...' : showSuccess ? 'Saved!' : 'Copy'}
        </span>
      </button>
