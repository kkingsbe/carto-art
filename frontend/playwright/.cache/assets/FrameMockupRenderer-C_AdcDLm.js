import { g as getDefaultExportFromCjs, r as reactExports } from './index-BGXLmG4_.js';
...
const Image$1 = createLucideIcon("image", __iconNode$d8);
...
function FrameMockupRenderer({
  ...
  const [compositeUrl, setCompositeUrl] = reactExports.useState(null);
    ...
    const compositeImages = async () => {
      ...
      try {
        ...
        stages.push({
          ...
          description: needsRotation ? "Design rotated 90° and scaled to cover the print area" : "Design scaled to cover the print area"
        });
        ...
        log(
          "Step 4: Sampled best color",
          `rgba(${targetR}, ${targetG}, ${targetB})`,
          "HSL:",
          `[${targetH.toFixed(1)}, ${targetS.toFixed(2)}, ${targetL.toFixed(2)}]`
        );
        ...
        stages.push({
          name: "4. Final Result",
          url: finalDataUrl,
          description: needsRotation ? "Final composited output (rotated back to original orientation)" : "Final composited output"
        });
        if (onDebugStages) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `relative flex items-center justify-center ${className}`, children: [
    ...
    isLoading && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute inset-0 flex items-center justify-center bg-muted/50 z-10 rounded-lg", children: /* @__PURE__ */ jsxRuntimeExports.jsx(LoaderCircle, { className: "w-8 h-8 animate-spin text-muted-foreground" }) }),
    error && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute top-0 left-0 right-0 bg-red-500/80 text-white p-2 text-xs z-20", children: error }),
...
function getProxiedUrl(src) {
  try {
    if (!src || src === "undefined") return "";
    const url = new URL(src);
    const needsProxy = url.hostname.includes("s3") || url.hostname.includes("amazonaws.com");
    if (needsProxy) {
...
function drawImageContain(ctx, img, targetArea) {
...
function drawImageCover(ctx, img, targetArea) {
  const { x, y, width, height } = targetArea;
  const imgRatio = img.width / img.height;
  const targetRatio = width / height;
...
export { FrameMockupRenderer, FrameMockupRenderer as default };
//# sourceMappingURL=FrameMockupRenderer-C_AdcDLm.js.map
