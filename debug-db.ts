...
const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!; // Use admin key if available, or assume RLS allows reading public flags
...
async function check() {
    ...
    if (error) {
        ...
        if (data && data.length > 0) {
            ...
            // Try inserting to see error? No, that's risky.
        }
    }
}
